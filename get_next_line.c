...
/*   Updated: 2025/02/11 18:26:52 by fyudris          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief Appends new content to the existing buffer and frees the
 old buffer.
 *
 ...
 * This function finds the first newline in the buffer and returns a newly
 allocated
 * string containing everything after that newline. The original buffer is
 freed.
 *
 ...
 *
 * This function extracts characters from the buffer up to and
 including the first
...
/**
 * @brief Reads content from a file descriptor until a newline is
 encountered.
 *
 * This function reads from the given file descriptor in chunks
 of size BUFFER_SIZE,
 * appending the read data to the accumulated result until a
 newline character is encountered
 * or there is nothing left to read.
 *
 * @param fd The file descriptor to read from.
 * @param result The current accumulated content.
 * @return A pointer to the accumulated content, or NULL if an
 error occurs.
 */
 ...
 *
 * This function uses a static buffer to keep track of previously
 read content.
 * It reads from the file descriptor until a newline is encountered,
 extracts the next
 * line from the buffer, updates the buffer with the remaining content,
 and returns the line.
 *
 * @param fd The file descriptor to read from.
 * @return The next line read from the file descriptor, or NULL if there
 is no more content or on error.
 */
